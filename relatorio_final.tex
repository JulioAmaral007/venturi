...
Para o problema do Venturi, o fluido é tratado como incompressível em regime permanente. As propriedades principais são:

\begin{itemize}
    \item \textbf{Densidade} $\rho$ (kg/m$^3$): relaciona massa e volume.
    \item \textbf{Viscosidade dinâmica} $\mu$ (Pa$\cdot$s): determina o regime de escoamento via número de Reynolds.
\end{itemize}

\textbf{No projeto}: em \texttt{app.py}, as propriedades são obtidas dinamicamente das bibliotecas \texttt{thermo.Chemical} (líquidos) e \texttt{thermo.Mixture} (ar), respeitando a dependência $\rho(T,P)$ e $\mu(T,P)$.
...
\textbf{No projeto}: no método \texttt{\_calcular\_desnivel\_de\_vazao}, após calcular $\Delta P$, o simulador converte a diferença de pressão em desnível manométrico, produzindo a leitura esperada do manômetro.
...
\textbf{No projeto}: em \texttt{VenturiSimulator.calcular}, após calcular as áreas $A_1$ e $A_2$, o método \texttt{\_calcular\_desnivel\_de\_vazao} implementa $v_1 = Q/A_1$ e $v_2 = Q/A_2$. A continuidade é aplicada na interface (\texttt{app.py}) para converter velocidade em vazão e no núcleo físico (\texttt{simulator.py}) para obter as velocidades.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Equação de Euler e Equação de Bernoulli}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Para escoamento entre as seções 1 (entrada) e 2 (garganta), em duto horizontal e fluido incompressível:
\[
    P_1 + \frac{1}{2}\rho V_1^2 = P_2 + \frac{1}{2}\rho V_2^2
\]

\textbf{No projeto (modo ideal)}: com $k_{\text{entrada}} = 0$, o código calcula:
\[
    P_2 = P_1 - \frac{1}{2}\rho\left(V_2^2 - V_1^2\right)
\]

\textbf{Extensão com perdas}: no modo realista ($k_{\text{entrada}} = 0{,}04$), a queda de pressão entre 1 e 2 é:
\[
    P_1 - P_2 = \frac{1}{2}\rho\left[(1 + k_{\text{entrada}})V_2^2 - V_1^2\right]
\]
correspondendo à equação de Bernoulli estendida com perda localizada.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Escoamento em dutos}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

O escoamento interno em dutos circulares está sujeito a perdas de carga distribuídas e localizadas:
\[
...
\[
    Re = \frac{\rho V D}{\mu}
\]
No projeto, calculado em \texttt{VenturiSimulator.calcular\_reynolds} e usado para classificar o regime e escolher a correlação de $f$.

\paragraph{Fator de atrito de Darcy}

Calculado pela função \texttt{calcular\_fator\_atrito}:
\begin{itemize}
    \item Se $Re < 2300$: regime laminar, $f = 64/Re$.
    \item Para $Re$ elevados: regime turbulento, utiliza-se a função \texttt{friction\_factor} da biblioteca \texttt{fluids}, que resolve numericamente a equação de Colebrook--White.
\end{itemize}

\paragraph{Perda distribuída na garganta}

Implementada pela equação de Darcy--Weisbach:
\[
    h_{f,\text{garganta}} = f\,\frac{L_{\text{garganta}}}{D_2}\,\frac{V_2^2}{2g}
\]

\paragraph{Perdas localizadas}

No modo realista, duas perdas localizadas são consideradas:
\begin{itemize}
    \item \textbf{Perda de entrada}: $k_{\text{entrada}} = 0{,}04$ no cálculo de $P_2$.
    \item \textbf{Perda no difusor}: coeficiente $K_{\text{difusor}}$ obtido em \texttt{\_obter\_k\_difusor\_15\_graus}, baseado na razão de áreas $AR = (D_1/D_2)^2$ e correlação empírica para difusores de $15^\circ$.
\end{itemize}

No balanço final de energia, a perda de carga total é obtida somando todas as perdas, resultando em $P_3 < P_1$ no modo realista.
